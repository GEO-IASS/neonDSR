function I=enviread(file_name,varargin)
%Input:
%file_name - input file name to be read in (this file can be with no
%extension or with a .img extension; the function will use this filename to
%look for the corresponding header file

%varargin{ ...} - function options, the option type followed by any needed
...
%10/3/2013 - fixed handling of file names, and endianness handling for bil,bsq formats - TCG

[fdir,fbase,fext] = fileparts(file_name);
hdrfile = fullfile(fdir,[fbase '.hdr']);
...
if nargin > 1
    ...
    while processedArgs < nargin
        ...
        switch lower(varargin{processedArgs-1})
            case {'lines'}
                processedArgs = processedArgs + 1;
                LinesToProcess = varargin{processedArgs-1};
            ...
            case {'samples'}
                processedArgs = processedArgs + 1;
                SamplesToProcess = varargin{processedArgs-1};
                samplesOptionFlag = 1;
                if strcmpi(info.interleave, 'bil') || strcmpi(info.interleave,  'bsq')
...
switch lower(info.interleave)
    ...
    case {'bsq'}
        ...
        for b=1:info.bands
            for i=1:info.lines
                t=fread(fid,info.samples,strcat('*',format),0,machine);
                tmp(i,:,b)=t;
            end
        end
