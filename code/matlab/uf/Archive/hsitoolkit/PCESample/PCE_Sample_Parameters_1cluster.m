function [Parameters] = PCE_Sample_Parameters_1cluster(X)
...
Parameters.NumberIterations = 300000;

%Parameters for Endmembers
Parameters.MaxNumEMs = 2;
...
Parameters.VarianceEndmemberSampleNarrow = mean(D(3,:))^2;
Parameters.EndmemberMixedDistWeightBroad = .4;
Parameters.df = size(X,2)+3;  %scale on inverse wishart for endmember prior covariance of each convex region

%Parameters for Sampling Partitions
Parameters.NumESamples = 0;
InitialNumberOfClusters = 1;
Parameters.Innovation = Parameters.NumESamples./(100*size(X,1));


%Parameters for Sample Trace Version & Endmember Clustering/Histogram
Parameters.CompressedSampleTraceFlag = 1;
Parameters.NumInitalSampleTraceLocations = 1;
...
for i = 1:InitialNumberOfClusters
    ...
    Parameters.beta = 100*mean(v(:)); %scale on the sampled covariance matrix
end
%-------------------------------------------------------------%
%-------------------------------------------------------------%
